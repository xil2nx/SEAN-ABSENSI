...
$today = date('Y-m-d');
$stmt = $pdo->prepare("SELECT s.nis, s.nama_siswa, k.nama_kelas, a.jam_datang, a.status 
                       FROM absensi a 
                       JOIN siswa s ON a.siswa_id = s.id 
                       LEFT JOIN kelas k ON s.kelas_id = k.id 
                       WHERE a.tanggal = ? AND a.status IN ('Izin', 'Sakit') 
                       ORDER BY a.status ASC, a.jam_datang ASC");
$stmt->execute([$today]);
...
<head>
    ...
    <title>Laporan Izin / Sakit Hari Ini</title>
...
<body class="bg-light p-3">
    <h3>Siswa Izin / Sakit Hari Ini (<?= date('d M Y') ?>)</h3>
    <a href="dashboard.php" class="btn btn-secondary mb-3">← Kembali</a>
    <div class="card">
        <div class="card-body">
            <?php if (empty($data)): ?>
                <div class="alert alert-info text-center">Tidak ada siswa Izin/Sakit hari ini.</div>
            <?php else: ?>
